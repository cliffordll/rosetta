#!/usr/bin/env node
/**
 * 把 `dist/rosetta-server.exe`(由 scripts/build.py 产出)同步到 Tauri 期待的
 * `tauri/binaries/rosetta-server-<host-triplet>.exe` 位置。
 *
 * Tauri 2 的 externalBin 用 rustc 当前 host triplet 作文件名后缀。本脚本通过
 * `rustc -vV` 读 host,避免硬编码。
 *
 * 被 `scripts/build.py --sync-sidecar` 和 `bun --filter=@rosetta/desktop sync-sidecar` 调用。
 */
...
const SRC_EXE = resolve(REPO_ROOT, "dist", "rosetta-server.exe");
...
function main() {
  ...
  const destExe = resolve(DEST_DIR, `rosetta-server-${triplet}.exe`);

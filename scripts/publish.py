...
"sidecar 必须跟主 exe 同目录"的硬约束;rosetta-server.exe 一份双重身份):
    ...
    └── rosetta-<ver>/
        ├── rosetta.exe                       CLI
        ├── rosetta-server.exe                server / 桌面 sidecar
        ├── rosetta-desktop.exe               桌面壳
        ├── Rosetta_<ver>_x64-setup.exe       NSIS installer(--installer 才有)
...
版本号 source-of-truth(7 处,bump 时同步,差一位 updater / installer 名都会乱):
    ...
    rosetta/__init__.py · __version__               · server /admin/status 输出
...
_TARGETS: list[_Target] = [
    ...
    _Target(
        _REPO_ROOT / "rosetta" / "__init__.py",
        _PY_DUNDER_VERSION,
        "rosetta/__init__.py __version__",
    ),
    _Target(
...
def _build_python_exes() -> None:
    """跑 scripts/build.py 产 dist/rosetta.exe + dist/rosetta-server.exe + sync sidecar。"""
    _run(
        [
            "uv",
            "run",
...
def _build_desktop(installer: bool) -> None:
    ...
    自动跑前端 `bun run --filter=@rosetta/app build`。
    """
...
def _collect(release_dir: Path, installer: bool) -> None:
    """把所有产物归集到 release_dir(dist/rosetta-<ver>/)。

    - dist/rosetta.exe / dist/rosetta-server.exe(build.py 直接写到 dist/ root)→ move
    - target/release/rosetta-desktop.exe → copy(rosetta-server.exe 内容跟上面那份
      一致,move 已搬走,不再重复)
    - target/release/bundle/nsis/{Rosetta_*_x64-setup.exe, latest.json} → copy(--installer)
    """
    ...
    desktop_src = _cargo_target_release() / "rosetta-desktop.exe"
    if not desktop_src.exists():
        raise RuntimeError(f"找不到桌面 exe:{desktop_src}")
    _copy(desktop_src, release_dir / "rosetta-desktop.exe")
...
def _cmd_build(installer: bool) -> int:
    ...
    release_dir = _DIST_DIR / f"rosetta-{version}"

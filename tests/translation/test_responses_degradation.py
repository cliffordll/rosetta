...
def test_target_responses_passthrough() -> None:
    ...
    result = degrade_responses_request(body, target_api=ServerApi.RESPONSES)
...
def test_previous_response_id_raises(target: ServerApi) -> None:
    body = _base() | {"previous_response_id": "resp_abc"}
    with pytest.raises(StatefulNotTranslatableError) as exc:
        degrade_responses_request(body, target_api=target)
...
def test_background_true_raises() -> None:
    ...
    with pytest.raises(StatefulNotTranslatableError) as exc:
        degrade_responses_request(body, target_api=ServerApi.MESSAGES)
    assert exc.value.field_name == "background"


def test_background_false_passes() -> None:
    body = _base() | {"background": False}
    result = degrade_responses_request(body, target_api=ServerApi.CHAT_COMPLETIONS)

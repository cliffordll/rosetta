...
> **文件定位**:`rosetta/cli/` 下用的 Typer 库、项目约定的命令写法、加新命令 / 新参数 / 新测试的手顺。
> **面向**:第一次改 `rosetta` CLI 子命令、或者想新增一条命令时查的自己。
...
| 入口在哪? | `rosetta/cli/__main__.py` 里的 `app = typer.Typer(...)` |
| 命令怎么注册? | 每个子命令文件 `rosetta/cli/commands/<name>.py` 写 `register(app)`,`__main__.py` 遍历调用 |
| 命令参数怎么定义? | 函数签名 + `Annotated[T, typer.Option(...)]` / `typer.Argument(...)` |
| 怎么跑? | `uv run rosetta <cmd>` · 打包后 `./dist/rosetta.exe <cmd>` |
...
| 参数 | 作用 |
|---|---|
| `name` | 帮助文本标题用的名字(与 `pyproject.toml` 的 `[project.scripts] rosetta = ...` 一致) |
| `help` | 根命令的一句话说明;`rosetta --help` 顶部那段 |
| `no_args_is_help=True` | 裸跑 `rosetta`(不带子命令)时打印 help,而不是报错退出。比 argparse 默认友好 |
...
例:`rosetta status`、`rosetta chat "hi"`

```python
# rosetta/cli/commands/status.py
import typer
...
2. `rosetta/cli/__main__.py` 顶部 import + 注册:
   ```python
   from rosetta.cli.commands import ping as ping_mod
   ...
   for mod in (..., ping_mod):
       mod.register(app)
   ```
...
4. 跑 `uv run rosetta ping` 看输出 / `uv run pytest tests/cli/test_commands.py -v` 验结构。
...
- 项目入口:`rosetta/cli/__main__.py`
- 项目命令实现:`rosetta/cli/commands/*.py`

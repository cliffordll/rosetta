...
- Tauri CLI 会自动跑 `beforeBuildCommand`(`bun --filter=@rosetta/app build`)
  确保前端 dist 最新,并把资源注入
- cargo 不管前端;手动 `bun --filter=@rosetta/app run build` 后再 `cargo build`
  也能,但多一步易漏
...
bun --filter=@rosetta/app run build
...
想把 exe 发给同事,**不经过安装**直接跑,拷贝这几项到目标机器任何目录:

```
my-rosetta/
├── rosetta-desktop.exe                                      # 主程序
└── binaries/
    └── rosetta-server-x86_64-pc-windows-msvc.exe           # sidecar
```

...
| 产 exe 自己双击 | `bun run tauri build --no-bundle --debug` | `target/debug/rosetta-desktop.exe`(~150 MB) | ~1-2 min |
| 产 exe 分发给别人 | `bun run tauri build --no-bundle` | `target/release/rosetta-desktop.exe`(~50 MB) | ~3-8 min |
...
bun run --filter=@rosetta/desktop sync-sidecar
```

`--sync-sidecar` 做的事:把 `dist/rosetta-server.exe` 拷贝到
`packages/desktop/tauri/binaries/rosetta-server-<target-triple>.exe`
(例:`rosetta-server-x86_64-pc-windows-msvc.exe`)。这是 Tauri 2 sidecar
...
- Server Python 改代码:需 `Ctrl+C` 停 `tauri dev` → 重跑 `--sync-sidecar`
  ...
  rosetta.server`(端口会不同,但 endpoint.json 机制兼容)
...
> `beforeBuildCommand` 里:必须写 `bun run --filter=@rosetta/app build`,
> 不能写 `bun --filter=@rosetta/app build`(后者会被解释成 `bun build`)。

产物:

- `target/release/rosetta-desktop.exe`(或 `target/debug/`)
- 前端静态资源 (HTML/CSS/JS)**内嵌进 exe**,不需单独目录
- 图标、部分 manifest 也在 exe 里

**注意**:sidecar(`rosetta-server.exe`)**不**内嵌,需要单独带。见 §5 分发。

### 为什么不直接 `cargo build`

- Tauri CLI 会自动跑 `beforeBuildCommand`(`bun run --filter=@rosetta/app build`)
  确保前端 dist 最新,并把资源注入
- cargo 不管前端;手动 `bun run --filter=@rosetta/app build` 后再 `cargo build`
  也能,但多一步易漏
...
bun run --filter=@rosetta/app build

cd packages/desktop/tauri
cargo build --release
./target/release/rosetta-desktop.exe
```
...
```
my-rosetta/
├── rosetta-desktop.exe   # 主程序
└── rosetta-server.exe    # sidecar(release build 时 Tauri 会自动从 binaries/<name>-<triple>.exe
                          #         拷到主 exe 同目录,并去掉 triple 后缀)
```

> ⚠️ **同目录是硬约束**。Tauri 2 的 `ShellExt::sidecar()` 在 release 模式下
> 只在主 exe 同目录找 `rosetta-server.exe`。只移动 `rosetta-desktop.exe`
...
**数据落点**:`~/.rosetta/rosetta.db` + `~/.rosetta/endpoint.json`
(全平台一致)。
...
| 关掉 app 后 `rosetta-server.exe` 残留 | `--parent-pid` watcher 5s 内自退 | `tasklist \| findstr rosetta-server` 等一会再看;5s 后仍在算 bug |
| debug / release exe 体积差很大 | debug 带全部符号,release 开 strip | 分发用 release;debug 自测快但别发给别人 |
| 双击 exe 秒闪退,无任何 UI | 主 exe 被单独移走,sidecar `rosetta-server.exe` 不在同目录 | release/ 里两个 exe 一起拷;见 §5 |

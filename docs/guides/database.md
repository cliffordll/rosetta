...
**自动创建,零人工 `createdb`**。SQLite 的 DB 文件首次连接时自动产生;rosetta server 启动时自动跑 migrations 生成 schema。

### 启动流程(`init_db()`)

在 `rosetta/server/app.py` 的 lifespan 钩子里调用:

```python
async def init_db(db_path: Path = DEFAULT_DB_PATH) -> None:
    db_path.parent.mkdir(parents=True, exist_ok=True)       # 确保 ~/.rosetta/ 存在
...
默认 DB 路径:`~/.rosetta/rosetta.db`(Windows 下是 `C:\Users\<you>\.rosetta\rosetta.db`)。
...
```bash
rm ~/.rosetta/rosetta.db
uv run python -m rosetta.server   # 下次启动自动按最新 migrations 重建
```
...
```python
# SELECT * FROM providers WHERE type = 'anthropic' AND enabled = 1
result = await session.execute(
    select(Provider)
    .where(Provider.type == "anthropic")
...
stmt = select(Provider).where(
    or_(
        and_(Provider.type == "openai", Provider.enabled.is_(True)),
...
```python
provider = await session.get(Provider, 1)   # Provider | None
if provider is None:
...
# 存在性检查
result = await session.execute(
    select(Provider.id).where(Provider.name == "x")
)
...
# 取 logs,同时预加载每条的 provider,避免 N 次额外 SQL
result = await session.execute(
    select(LogEntry)
    .options(selectinload(LogEntry.provider))
    .order_by(LogEntry.created_at.desc())
    .limit(20)
)
logs = result.scalars().all()
# logs[0].provider.name 直接可访问,无额外 SQL
```

**注**:需要在 `LogEntry` 模型里先声明 `relationship("Provider")`,当前 v0 还没加(按需)。
...
await session.refresh(provider)  # 拿回 DB 生成的 id 和 created_at
print(provider.id)               # 1
```
...
```python
session.add_all([Provider(...), Provider(...), Provider(...)])
...
```python
provider = await session.get(Provider, 1)
if provider is None:
    raise HTTPException(404)

provider.enabled = False
provider.api_key = "sk-new"
...
await session.execute(delete(Provider).where(Provider.id == 1))
await session.commit()
```
...
try:
    session.add(Provider(name="duplicate", ...))
    await session.commit()
...
典型场景:v0.1 已有用户 20 条 provider 记录,v0.2 要加 `last_used_at` 字段:
- 不管 → 代码用新字段,DB 没有 → 运行时 `no such column` 报错
- 删库重建 → 数据丢
- **Migration** → `ALTER TABLE providers ADD COLUMN ...`,数据保留 ✓
...
### 6.3 加字段的完整 5 步(以 `providers.last_used_at` 为例)
...
ALTER TABLE providers ADD COLUMN last_used_at TEXT;
...
#### 步骤 4(可选):暴露到 API(`admin/providers.py`)
...
敏感字段(`api_key`)**不应**出现在 `ProviderOut` 里。
...
ALTER TABLE providers RENAME COLUMN type TO provider_type;
...
UPDATE providers SET enabled = 1 WHERE enabled IS NULL;
...
举例:给 `providers` 加 `last_used_at` 列是 migration;把 provider=5 的 `last_used_at` 改成 `NOW()` 是业务 UPDATE。
...
- **session**:基于 connection 的高层抽象,管事务边界、对象状态、identity map(同一 id 的 Provider 对象在一个 session 里只有一份)

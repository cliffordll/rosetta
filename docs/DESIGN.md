...
| 认证 | **无 API-level auth**（loopback-only 隔离）；`x-api-key` / `Authorization: Bearer` 若传则透传给上游作 override，不传用 `providers.api_key` | token（从 `endpoint.json` 读，仅防跨用户误触） |
...
│   │   │   ├── providers.py      # /admin/providers CRUD（ProviderCreate / ProviderOut 内联）
...
│   │   │   ├── selector.py       # pick_provider：按 x-rosetta-provider header 选 provider
│   │   │   └── exceptions.py     # ServiceError(status, code, message, **extra) domain exception
│   │   │
│   │   ├── repository/           # data access 层：ORM 查询封装
│   │   │   ├── __init__.py       # re-export + ProviderRepoDep / LogRepoDep FastAPI 依赖别名
│   │   │   ├── provider.py       # ProviderRepo(list_all / get_by_id / get_by_name / count / create / delete)
│   │   │   └── log.py            # LogRepo(list_with_provider / aggregate_stats)
│   │   │
│   │   ├── database/             # infra：engine / session / models / migrations
│   │   │   ├── __init__.py
│   │   │   ├── models.py         # SQLAlchemy 2.x 声明式 Base + Provider / LogEntry
...
│   │   │   ├── messages/         # Format.MESSAGES（Anthropic Messages API）
...
│           ├── provider.py       # rosetta provider add/list/remove/test
...
GET    /v1/models                        合并所有 enabled provider 的可用模型列表
                                         ...
                                         可选过滤：?provider=<name> 只返回该 provider 的模型
                                         响应中每条 model 额外带 "provider": "<name>" 字段，方便前端自行按 provider 分组过滤
```
...
- 数据面（`/v1/*`）的 `x-api-key` / `Authorization: Bearer` **不是用来验 rosetta 身份的**，它是**上游 key 的透传槽**：
  ...
  - 客户端请求里**没带** → fallback 到 `providers.api_key`（DB 里预先配置的那一把）
- 管理面（`/admin/*`）的 token 仍从 `endpoint.json` 读，仅作跨用户误触防护（同机不同 user 别乱连），不是安全边界

**数据面必需 header(仅对 `/v1/*` 生效)**:

- `x-rosetta-provider: <name>` — **必须**指定上游 provider(按 name 精确匹配)。缺失 / provider 不存在 / disabled 都返回 400。rosetta 不做 model-based 自动路由,客户端需自行选 provider(CLI `rosetta chat --provider`、Chat 页、外部 SDK 都统一靠这个 header)。
...
**`providers.base_url` 默认值**（type 对应的官方上游，建 provider 时 `base_url` 留空就取这里）：
...
| `custom` | **必须显式填写**，否则 `POST /admin/providers` 返回 400 | 国内中转站、自托管 vLLM / Ollama、第三方网关 |
...
客户端 JSON
  ...
  → [selector] 按 x-rosetta-provider header 选 provider
...
**rosetta 不做 model-based 自动路由**,客户端必须通过 `x-rosetta-provider: <name>` header 显式指定 provider。原设计里按 `model_glob` 自动匹配 provider 的 `routes` 表已经移除(2026-04 简化),单个 http 请求的选 provider 流程只剩一步:

```
1. header 有 x-rosetta-provider: <name>?
   · 有 → 按 name 精确匹配 provider
      · 找到 enabled → 用它
      · 不存在 → 400 provider_not_found
      · 被禁用 → 400 provider_disabled
   · 没有 → 400 missing_rosetta_provider
```

**关于选中的 provider 与入口 format**:选中 provider 的 `type` 对应的格式与入口 format 不一致时,自动走 §8.3 翻译(对角线直通仅发生在两者一致的情况)。例如入口 `/v1/messages` + provider `type=openai` → IR 翻译为 Chat Completions 请求。
...
```json
{
  "error": {
    "type": "rosetta_error",
    "code": "provider_not_found",   // 或 missing_rosetta_provider / provider_disabled
    "message": "x-rosetta-provider 指定的 'ghost' 不存在"
  }
}
```
...
**客户端配合**:
- `rosetta chat --provider <name>` 自动注入 header
- GUI Chat 页 "Provider 下拉" 必选,每次发送都带 header
- 外部 SDK 调用:应用层在 HTTP client 里统一加 header(provider 是"环境配置")

### 8.5 端到端请求链路（以 CLI chat 为例）

把前面几节串起来，跟一遍 `rosetta chat --format messages "hi" --model claude-haiku-4-5` 从敲下命令到拿到第一个 token 的完整路径。（演示的是**正常模式**，客户端**不传** `--api-key`，靠 server 里存的 `providers.api_key` 兜底。）
...
└──────┬──────┘                 └──────────┬────────────┘                └────────┬─────────┘
       ...
       │                                    │ 4. 按 x-rosetta-provider header     │
       │                                    │    选 provider "anthropic-main"     │
       │                                    │ 5. 读 providers 行:                 │
       │                                    │    base_url → https://api.anth…     │
       │                                    │    api_key  → sk-ant-XXXX           │
       │                                    │ 6. 解析上游 api-key:                │
       │                                    │    请求头有 x-api-key? 用它         │
       │                                    │    没有 → 用 providers.api_key      │
       │                                    │    （这里走"没有"分支）              │
       ...
       │ ◄───────────────────────────────── │                                     │
       │                                    │ 10. 异步写 logs                     │
       │ 11. 按 format 解码，逐 token 打印  │     (provider_id, tokens, latency)  │
```
...
| 字段 | 链路步骤 | 来源 | 去向 | 备注 |
|---|---|---|---|---|
| 上游 URL | 8 | `providers.base_url`（空则按 type 默认，见 §8.2） | httpx 请求行 | 用户在 GUI / `provider add` 填一次 |
| 上游 api-key | 8 | **客户端 `x-api-key` 头**（若带）→ **否则 `providers.api_key`** | httpx 请求头（按上游 type 选具体写法） | v0 无"rosetta 本地 key"概念 |
| `model` | 2 → 8 | CLI `--model` 参数 | 上游 `body.model` 原样 | v0 不做别名翻译 |
| `messages[]` | 2 → 8 | CLI 内存数组（多轮历史） | 上游 body（直通）或 adapter 翻译后（跨格式） | |
| `x-rosetta-provider: foo`（**必须**） | 2 | CLI `--provider foo` | server 按 name 查 provider | **不转发上游**;缺失直接 400 |

#### 客户端显式传 `--api-key` 的分支

`rosetta chat --api-key sk-XYZ --model claude-haiku-4-5 "hi"`：在步骤 2 的请求里带上 `x-api-key: sk-XYZ`，步骤 6 检测到就用这把，**不**读 `providers.api_key`。其他步骤完全不变。这条分支让你"临时用一把不一样的 key 试试"不需要改任何 DB 配置。
...
- **不路由**:没有 `providers` / `x-rosetta-provider` 概念介入。
...
| 与 `--base-url` 互斥 | 原因 |
|---|---|
| `--provider <name>` / `x-rosetta-provider` 头 | direct 模式没有 provider 概念 |
...
[direct · api.anthropic.com · claude-haiku-4-5 · 8→18 tokens · 412ms]
 ↑ direct 模式用 hostname 作为"provider"位，因为没有 name 可取
```
...
| 经 server | 是 | 否 |
| api-key 来源 | `x-api-key` 头若有则 override，否则 `providers.api_key` fallback | 必填，直接走 `x-api-key` / `Authorization` |
| URL 来源 | `providers.base_url`（按 `x-rosetta-provider` header 查 name） | `--base-url` 直填 |
...
Pydantic 模型直接 `from rosetta.server.schemas.admin import Provider` 复用，**不手写第二份**——单包结构下子包互相 import 无需包依赖声明。
...
rosetta chat --format completions        # 入口格式，默认 messages（messages | completions | responses）
                                         #   分别对应 /v1/messages、/v1/chat/completions、/v1/responses
rosetta chat --provider foo "hi"         # 必填:指定 provider(带 x-rosetta-provider header)
rosetta chat --api-key sk-ant-XXX "hi"   # 可选:覆盖 providers.api_key，临时用另一把上游 key
...
- **鉴权**：CLI **不持有任何 rosetta 自有的本地 key**——server 不做 API-level auth（loopback-only）。`--api-key` 是**可选的上游 key override**：传了就附加 `x-api-key: <你的值>` 让 server 透传；不传就让 server 用 `providers.api_key`。direct 模式下 `--api-key` 是必填。
...
# ─── step 2：加一个真实上游 provider ─────────────────────────────
$ rosetta provider add \
    --name anthropic-main \
    --type anthropic \
    --api-key sk-ant-api03-XXXXXXXXXXXXXXXX
→ POST /admin/providers，server 写 SQLite（base_url 留空→默认 https://api.anthropic.com）
provider "anthropic-main" created (id=1, base_url=https://api.anthropic.com)

# ─── step 3：确认状态 ──────────────────────────────────────────
$ rosetta status
server:    http://127.0.0.1:62538  (uptime 12s)
providers: 1 enabled (anthropic-main)

# ─── step 4：一次性对话（最小链路自检）──────────────────────────
$ rosetta chat --provider anthropic-main "用一句话介绍你自己"
→ POST /v1/messages to 127.0.0.1:62538
   header: x-rosetta-provider: anthropic-main
   （无 x-api-key，让 server 用 providers.api_key 兜底）
   body.model: claude-haiku-4-5          ← 内置默认模型（type=anthropic→haiku）
→ server: 按 header 选中 anthropic-main → 用 providers.api_key sk-ant-XXX
...
# ─── step 6：加第二个 provider + 跨格式翻译验证 ──────────────────
$ rosetta provider add --name openai-main --type openai --api-key sk-XXX

# CLI 用 messages 格式打 OpenAI → server 走翻译 messages→IR→completions
$ rosetta chat --provider openai-main --model gpt-4o-mini "hi"
Hello! How can I help you today?
[openai-main · gpt-4o-mini · 8→9 tokens · 402ms · messages→IR→completions]

# CLI 用 completions 格式打 OpenAI → 对角线直通
$ rosetta chat --provider openai-main --format completions --model gpt-4o-mini "hi"
Hello! How can I help you today?
[openai-main · gpt-4o-mini · 8→9 tokens · 398ms · completions↔completions 直通]

# 切回 Anthropic provider
$ rosetta chat --provider anthropic-main --format messages --model claude-haiku-4-5 "ping"
pong
[anthropic-main · claude-haiku-4-5 · 4→2 tokens · 312ms · messages↔messages 直通]

# ─── step 7：临时换一把 api-key 测试（不修改 provider）──────────────
$ rosetta chat --provider anthropic-main --api-key sk-ant-OTHER --model claude-haiku-4-5 "hi"
  ↑ server 收到请求头带 x-api-key，就拿这把去打上游，不读 providers.api_key
...
1. **server 没有自己的 key 概念**——step 4 里 CLI 没传 `x-api-key`，server 用 providers 里存的；step 7 里 CLI 传了,server 就透传。概念统一。
...
4. **provider 必须显式指定**:rosetta 不做 model-based 自动路由,客户端每次都带 `x-rosetta-provider` header(`--provider <name>`);没带直接 400。
...
3. **Logs** — 请求列表 + 按 provider / 时间筛选
4. **Chat** — 多轮对话测试页，provider / format / model 可选，流式渲染；消息保存在**当前页面的 React state**，切页或刷新即清（v0 不做持久化，见下节）

（v0 **没有 Keys 页**——rosetta 不再有自有的"本地 key"概念，上游 api-key 在 Providers 页编辑 provider 时填入。见 §8 鉴权说明。）
...
```
┌─ Chat ────────────────────────────────────────────┐
│ Provider: [anthropic-main ▾]  Format: [messages ▾] │
...
- **Provider 下拉**:必填,挂载时拉 `GET /admin/providers` 填充选项;每次发送都带 `x-rosetta-provider: <name>` header。没选 provider 不能点发送。
...
- **Model 下拉**:来源 `GET /v1/models?format=<当前>&provider=<Provider 下拉值>`,随 format / provider 联动。
- **流式**：浏览器 `fetch` + `ReadableStream` 读 SSE，按当前 format 解码后逐 token 追加到 assistant 气泡；`Stop` 按钮 `AbortController.abort()`。
- **鉴权**：前端对本地 server **不带任何 rosetta 自有的 key**——server 只绑 loopback,不做 API-level auth。`/v1/*` 请求默认**不加 `x-api-key` 头**,让 server 用 `providers.api_key` 兜底。可选的"临时换 key 测试"场景在页面右上角提供一个小入口("Override api-key"),存会话内存、不落盘、切页即清。
...
```
┌─ Dashboard ────────────────────────────────┐
│ ⚠ 当前没有启用的 provider，无法发起对话     │
│                                            │
│      [ + 添加 Provider ]                   │
└────────────────────────────────────────────┘
```

**③ 点按钮跳 Providers → 弹 Modal 填第一个 provider**
...
Save 后 `POST /admin/providers` → 写 SQLite。Dashboard 顶部警告消失。
...
fetch(`${url}/v1/messages`, {
  method: 'POST',
  headers: {
    // 默认不带任何鉴权头，server 会用 providers.api_key 兜底。
    // 用户在右上角 "Override api-key" 输入了临时 key 时才加这一行：
    //   'x-api-key':        <override key>,
    'x-rosetta-provider': <Provider 下拉选中的 name>,   // 必填
...
- meta 行变成 `messages→IR→completions` 或 `completions↔completions`，取决于被路由到哪个 provider 类型
...
直接定位"上游鉴权失败" → 用户回 Providers 页改 key → 重试。无需 tail 日志。

...
│           ├── route.py          # ▣ NEW  rosetta route list/add/remove
...
**数据面可选 header（仅对 `/v1/*` 生效）**：

- `x-rosetta-provider: <name>` — 强制指定上游 provider（按 name 精确匹配），跳过 `routes` 匹配。provider 不存在 / disabled 返回 400。Chat 页和 `rosetta chat --provider` 用它。
...
2. header 有 x-rosetta-provider: <name>？按 name 直接取该 provider（跳过 3-5）
3. 按 priority ASC 遍历 routes 表
4. 第一个 model_glob 匹配的 → 用那条的 provider_id
5. 都不匹配 → 用第一个 enabled 的 provider
6. 该 provider 被禁用 / 不存在 → 503
7. 连一个 enabled provider 都没有 → 503 error_code=no_provider_available
```

**关于选中的 provider 与入口 format**：无论是哪条 rule 选出的 provider，**选中 provider 的 `type` 对应的格式与入口 format 不一致时，自动走 §8.3 翻译**（对角线直通仅发生在两者一致的情况）。例如入口 `/v1/messages` + provider `type=openai` → IR 翻译为 Chat Completions 请求。这条对 rule 2(`x-rosetta-provider` 指定)、rule 4(glob 匹配)、rule 5(兜底) 一视同仁。
...
```json
{
  "error": {
    "type": "rosetta_error",
    "code": "no_provider_available",   // 或 "provider_disabled"
    "message": "no enabled provider matches model 'gpt-4'"
  }
}
```
...
| `x-rosetta-provider: foo`（可选） | 2 | CLI `--provider foo` | server 绕 routes 查 provider | **不转发上游** |
...
| URL 来源 | `providers.base_url`（按 provider name / routes 查表） | `--base-url` 直填 |
...
# route 管理
rosetta route list
rosetta route add --pattern "claude-*" --provider anthropic-main [--priority 1]
rosetta route remove <id>
...
rosetta chat --provider foo "hi"         # 绕过 routes 指定 provider（带 x-rosetta-provider header）
...
routes:    0 rules  ← 无路由规则，按 §8.4 rule 5 兜底到"第一个 enabled provider"

# ─── step 4：一次性对话（最小链路自检）──────────────────────────
$ rosetta chat "用一句话介绍你自己"
...
→ server: 无 route → 落到 anthropic-main → 用 providers.api_key sk-ant-XXX
...
$ rosetta route add --pattern "claude-*" --provider anthropic-main --priority 1
$ rosetta route add --pattern "gpt-*"    --provider openai-main    --priority 2
  ↑ 有多 provider 后必须显式路由；否则仍按"第一个 enabled"兜底

# CLI 用 messages 格式打 OpenAI → server 走翻译 messages→IR→completions
$ rosetta chat --model gpt-4o-mini "hi"
Hello! How can I help you today?
[openai-main · gpt-4o-mini · 8→9 tokens · 402ms · messages→IR→completions]

# CLI 用 completions 格式打 OpenAI → 对角线直通
$ rosetta chat --format completions --model gpt-4o-mini "hi"
Hello! How can I help you today?
[openai-main · gpt-4o-mini · 8→9 tokens · 398ms · completions↔completions 直通]

# 绕过 routes 强制打某个 provider（带 x-rosetta-provider header）
$ rosetta chat --provider openai-main --format messages --model gpt-4o-mini "ping"
pong
[openai-main · gpt-4o-mini · 4→2 tokens · 312ms · messages→IR→completions]

# ─── step 7：临时换一把 api-key 测试（不修改 provider）──────────────
$ rosetta chat --api-key sk-ant-OTHER --model claude-haiku-4-5 "hi"
...
4. **路由优先级**：只有一个 provider 时兜底；多 provider 时必须显式 route，否则所有模型都打第一个 provider（可能是错的）。
...
4. **Logs** — 请求列表 + 按 provider / 时间筛选
5. **Chat** — 多轮对话测试页，provider / format / model 可选，流式渲染；消息保存在**当前页面的 React state**，切页或刷新即清（v0 不做持久化，见下节）
...
- **Provider 下拉**：默认"自动路由"（不加 header）；选中具体 provider 时每次发送带 `x-rosetta-provider: <name>`，便于定点测试某个 provider。
...
- **Model 下拉**：来源 `GET /v1/models?format=<当前>`，随 format 联动；响应中每条 model 带 `provider` 字段，Provider 下拉非"自动路由"时前端 client-side 按该字段过滤（也可在请求加 `?provider=<name>` 让 server 过滤，二选一，前者省一次网络请求）。
...
fetch(`${url}/v1/messages`, {
  ...
  headers: {
    ...
    'x-rosetta-provider': <Provider 下拉选中的 name，自动路由时省略>,
